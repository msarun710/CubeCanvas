...
def p2_image_display(app):
    ...
    # Crosshair follows normal mouse movement
    app.image_canvas.bind("<Motion>", app.move_crosshair)

    # Hide crosshair when the cursor leaves the canvas
    app.image_canvas.bind("<Leave>", app.hide_crosshair)

    # Start crop selection on left mouse press
    app.image_canvas.bind("<Button-1>", app.start_crop)

    # Resize the crop rectangle while dragging
    app.image_canvas.bind("<B1-Motion>", app.update_crop)

    # Keep the crosshair moving during drag operations.
    # Without add="+", this binding would replace update_crop().
    app.image_canvas.bind(
        "<B1-Motion>",
        app.move_crosshair,
        add="+"
    )

    # Finalize the crop rectangle when the mouse is released
    app.image_canvas.bind(
        "<ButtonRelease-1>",
        app.finish_crop
    )
