...
class App(ctk.CTk):
    ...
    def confirm_crop(self, event=None):
        """
        Finalize the crop operation.

        Steps:
        1. Convert the crop rectangle from canvas coordinates
        back to the original image coordinates.
        2. Crop the original image.
        3. Store both permanent and working copies of the crop.
        4. Build the Rubik colour palette from user inputs.
        5. Generate the initial mosaic.
        6. Display the preview page.
        """

        # No crop has been selected yet
        if self.crop_display_coords is None:
            return

        # Crop coordinates on the displayed image
        left, top, right, bottom = self.crop_display_coords

        # Determine how much the original image was scaled
        # when it was displayed on the canvas
        scale_x = (
        ...
        # Convert display coordinates back to
        ...
        # This is never modified and allows
        # effects to be reapplied from scratch.
        self.original_cropped_image = (
            self.cropped_image.copy()
        )

        # Working image used for page 3 previews.
        # Effects will modify this copy.
        self.preview_image = (
            self.cropped_image.copy()
        )

        #
        # Build Rubik palette from colour entries
        #
        palette = []
        ...
        for _, entry in self.color_widgets:
            ...
            try:
                # Remove '#' from hexadecimal colour
                ...
                # Ignore invalid colour entries
                pass

        # Convert palette to NumPy for faster processing
        palette = np.array(
            palette,
            dtype=np.uint8
        )

        # Save palette for future effect updates
        self.palette = palette

        #
        # Read mosaic dimensions
        #
        rows = int(self.rows_entry.get())
        cols = int(self.cols_entry.get())
        ...
        # Update page 3 preview canvases
    ...
    def fit_image_canvas(self, image, canvas, force_size=None):
        """
        Display an image inside a canvas while preserving
        its aspect ratio.
        ...
        force_size : tuple(width, height), optional
            If provided, resize the image to these exact
            dimensions instead of automatically fitting
            it to the canvas.
        """
        ...
        # Automatically fit image to canvas
        ...
        # Use supplied dimensions
        else:
            nw, nh = force_size

        # NEAREST preserves pixel art appearance
        resized = image.resize(
            (nw, nh),
        ...
        # Prevent Tkinter from garbage collecting
        # the image reference
        canvas.image_ref = tk_img

        # Display image centred inside canvas
        canvas.create_image(
            (w - nw) // 2,
            (h - nh) // 2,
            anchor="nw",
    ...
    def update_page3_images(self, event=None):
        """
        Refresh both preview canvases on Page 3.

        The original preview image and processed
        mosaic are displayed using identical display
        dimensions so they can be compared directly.
        """

        if not hasattr(self, "cropped_image"):
            return

        # Ensure canvas size information is current
        ...
        # Display processed mosaic using
    ...
    def apply_page3_effects(self):
        """
        Apply all page 3 effects and regenerate
        the mosaic preview.

        Effects are always applied to the untouched
        original crop so that adjustments are not
        compounded repeatedly.
        """

        # Stop execution pathways immediately if structural image assets have not been generated in active memories.
        if not hasattr(self, "original_cropped_image"):
            return

        # Gather standard row and column counts from form text objects to maintain correct scale dimensions.
        rows = int(self.rows_entry.get())
        cols = int(self.cols_entry.get())

        #
        # Rebuild preview image from scratch
        #
        ...
        #
        # Optional blur applied after mosaic generation
        #
    ...
    def revert_changes(self):
        """
        Restore all Page 3 controls to their
        default values and regenerate previews.
        """
