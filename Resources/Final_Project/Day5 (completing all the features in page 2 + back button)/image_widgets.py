...
def make_page2(app):
    # Bind the window resize trigger directly to page 2 frame modifications to properly update rendered image content.
    ...
    app.page2.grid_rowconfigure(1, weight=12)  # main area
...
def p2_topbar(app):
    ...
    app.topbar.grid_columnconfigure(0, weight=0)   # Rows
    app.topbar.grid_columnconfigure(1, weight=0)   # rows entry
    app.topbar.grid_columnconfigure(2, weight=0)   # × Columns
    app.topbar.grid_columnconfigure(3, weight=0)   # cols entry
    app.topbar.grid_columnconfigure(4, weight=0)   # total label
...
def p2_cube_colors(app):
    ...
    for row, color in enumerate(default_colors):
        ...
        # Insert "#FFFFFF"
        # starting at index 0 (just saying add it at start)
        ...
        # add = lambda a, b: a + b    outputs the sum a+b
        entry.bind("<KeyRelease>",
                   lambda event, box=color_box, ent=entry: update_color_box(box, ent)
                )
...
# Refresh the background frame shading properties dynamically within the settings panel area when inputs are modified.
def update_color_box(color_box, entry):
    ...
    color = entry.get()   # this will whatever is written inside the entry box (the hexcode)
    ...
    try:
        ...
        color_box.configure(fg_color=color)   # this will run only if it is a valid hex code for any color
    except:
...
def p2_image_display(app):
    ...
    # Initialize the actual image rendering engine context, configuring a basic fallback indicator message for safety.
    ...
    # Connect internal canvas resizing instructions directly to the system layout update script using configure event hooks.
