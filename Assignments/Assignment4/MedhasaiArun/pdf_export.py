...
CUBE_LINE = (18, 24, 32)
CUBES_PER_CHUNK = 3  # Standard: Each printable chunk page spans 3x3 cubes
...
# FIX 1: Cache fonts in memory so Python doesn't open disk files thousands of times
...
# FIX 2: Correct chunk sticker size calculation (3 cubes per chunk page)
def chunk_counts(image, cube_size=3):
    chunk_sticker_size = max(1, cube_size * CUBES_PER_CHUNK)
    return math.ceil(image.width / chunk_sticker_size), math.ceil(image.height / chunk_sticker_size)


def chunk_bounds(image, col, row, cube_size=3):
    chunk_sticker_size = max(1, cube_size * CUBES_PER_CHUNK)
    x1 = col * chunk_sticker_size
    y1 = row * chunk_sticker_size
    x2 = min(x1 + chunk_sticker_size, image.width)
    y2 = min(y1 + chunk_sticker_size, image.height)
    return x1, y1, x2, y2


def draw_chunk_grid(draw, image, placed_box, current=None, show_labels=True, cube_size=3):
    # Draw chunk lines using pixel scale, not row/column averages
    x, y, displayed_w, displayed_h = placed_box
    scale_x = displayed_w / image.width
    scale_y = displayed_h / image.height
    cols, rows = chunk_counts(image, cube_size=cube_size)

    for row in range(rows):
        for col in range(cols):
            x1, y1, x2, y2 = chunk_bounds(image, col, row, cube_size=cube_size)
            box = [
                x + x1 * scale_x,
                y + y1 * scale_y,
                x + x2 * scale_x,
...
def create_overview_page(image, cube_size=3):
    # Build the PDF cover page as an explicit guide
    ...
    stats = [
        ...
        f"Cube size: {cube_size}x{cube_size}",
    ]
    ...
    draw_chunk_grid(draw, image, placed, show_labels=True, cube_size=cube_size)
...
def draw_instruction_grid(draw, chunk, box, cube_size=3):
    ...
    cell = max(18, min(available_w // max(1, chunk.width), available_h // max(1, chunk.height)))
...
def create_page(chunk, label, full_image, row, col, cube_size=3):
    ...
    draw_chunk_grid(draw, full_image, placed, current=(row, col), show_labels=True, cube_size=cube_size)

    draw_instruction_grid(draw, chunk, (85, 575, 1115, 1085), cube_size=cube_size)

    return page


def export_pdf(image, cube_size=3):
    # Normalize to RGB so every PDF page can be saved without mode-specific errors.
    image = image.convert("RGB")
    chunk_cols, chunk_rows = chunk_counts(image, cube_size=cube_size)
    pages = [create_overview_page(image, cube_size=cube_size)]
