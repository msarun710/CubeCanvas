...
def p3_topbar(app):
    ...
    # Add dropdown next to Export button
    app.cube_type_optionmenu = ctk.CTkOptionMenu(
      app.p3_topbar,
      values=["3x3", "4x4", "5x5", "6x6", "7x7"],
      command=lambda choice: app.apply_page3_effects(),
      width=80
    )
    app.cube_type_optionmenu.set("3x3")
    app.cube_type_optionmenu.grid(row=0, column=4, padx=5, pady=5)
...
def p3_left_panel(app):
    ...
    # --- Pixelation Algorithm Dropdown ---
    algo_label = ctk.CTkLabel(app.p3_left, text="Pixelation Algorithm", font=("Arial", 12, "bold"))
    algo_label.pack(anchor="w", padx=10, pady=(10, 2))

    app.algorithm_optionmenu = ctk.CTkOptionMenu(
        app.p3_left,
        values=["Nearest Color", "Floyd-Steinberg", "Ordered (Bayer)", "Atkinson Dithering"],
        command=lambda choice: app.apply_page3_effects()
    )
    app.algorithm_optionmenu.set("Nearest Color")
    app.algorithm_optionmenu.pack(fill="x", padx=10, pady=(0, 10))
    ...
    slider_configs = [
        ...
        ("Gamma", "gamma_slider", 0.2, 3.0, 1.0),
    ]
    ...
    # 4. Invert (Negative) --- [NEW]
    app.invert_switch = ctk.CTkSwitch(
        switch_frame,
        text="Invert (Negative)",
        command=app.apply_page3_effects
    )
    app.invert_switch.pack(anchor="w", padx=15, pady=8)
    ...
    app.grayscale_switch.pack(anchor="w", padx=15, pady=8)
    ...
    app.wb_switch.pack(anchor="w", padx=15, pady=(8, 15))

    # Container frame for Undo / Redo buttons
    history_frame = ctk.CTkFrame(app.p3_left, fg_color="transparent")
    ...
    undo_btn.pack(side="left", expand=True, padx=(0, 5))
    ...
    redo_btn.pack(side="right", expand=True, padx=(5, 0))

    # Construct a button to display the RGB histogram of the edited image
    app.histogram_button = ctk.CTkButton(
        app.p3_left,
        text="Show RGB Histogram",
        command=app.show_histogram_action
    )
